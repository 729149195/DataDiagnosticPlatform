  ...
  try {
    ...
    function selectionBrushed(event) {
      ...
      g.select('.selection-brush .overlay').style(
          'pointer-events',
          isBoxSelect.value ? 'all' : 'none'
      );

      g.select('.selection-brush .selection').style('display', isBoxSelect.value ? null : 'none');
    ...
    function drawAnomalyElements(anomaly, anomaliesGroup, isStored = false) {
      ...
      const buttonGroup = anomalyGroup
          ...
          .style('pointer-events', isBoxSelect.value ? 'all' : 'none')
          .style('opacity', isBoxSelect.value ? 1 : 0.5);
