<template>
  <div class="chart-container">
    <div v-for="(row, rowIndex) in channelsInRows" :key="'row-' + rowIndex" class="chart-row">
      <div class="chart-wrapper" v-for="(channel, index) in row" :key="channel.channel_name + '_' + channel.shot_number">
        <div :id="'chart-' + channel.channel_name + '_' + channel.shot_number" :ref="el => channelSvgElementsRefs[index + rowIndex * 2] = el" :style="{
...
// 新增：每行两个通道分组
const channelsInRows = computed(() => {
  const rows = [];
  for (let i = 0; i < selectedChannels.value.length; i += 2) {
    rows.push(selectedChannels.value.slice(i, i + 2));
  }
  return rows;
});
  ...
  requestAnimationFrame(() => {
    try {
      ...
      if (titleElement) {
        ...
        // 获取颜色选择器容器（只在当前chart-wrapper下查找）
        const channelKey = `${channel.channel_name}_${channel.shot_number}`;
        const chartWrapper = chart.container.closest('.chart-wrapper');
        const colorPickerContainer = chartWrapper?.querySelector('.color-picker-container');
...
.chart-wrapper {
  ...
  padding: 5px 0;
  margin-bottom: -10px;
  /* 不要加 display:flex 或 flex:1 或 flex-direction: column 或 width: 100% */
}
