...
def find_true_segments(bool_array):
    ...
    if bool_array[-1] == True:
        ends = np.append(ends, len(bool_array))
...
def period_condition_anomaly(channel_name, period, condition, mode):
    ...
    is_anomaly = condition_judge(period_data, condition, mode)
    anomaly_time = [period] if is_anomaly else []
    return anomaly_time
